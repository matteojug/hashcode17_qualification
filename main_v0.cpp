...
void update_scores(){
    ...
    for (auto r : req){
        ...
        for (auto c : cc[r.ff.ss]){
            ...
            if (video_size[r.ff.ff] > cache_size[c.ff]) continue;
    ...
    for (int i = 0; i < (int)rxc.size(); i++){
        auto c = rxc[i];
        for (auto v : c){
            pq.push(mp( v.ss / pow(video_size[v.ff],EXP), mp(i,v.ff)));
        }
    }
}
...
int main(int argc, char *argv[]){
    ...
    update_scores();
    int cnt = 0, ccnt = 0;
    int FHKGJDKJLFBG[] = {84,36200,9000,40000};
    while (!pq.empty()){
        auto t = pq.top(); pq.pop();
        //~ cout<<t.ff<<","<<t.ss.ff<<","<<t.ss.ss<<endl;
        //~ cout<<video_size[t.ss.ss]<<","<<cache_size[t.ss.ff]<<endl;
        if (video_size[t.ss.ss] > cache_size[t.ss.ff]) continue;
        ccnt++;
        //~ if (changed.count(t.ss.ss)){ cnt += FHKGJDKJLFBG[FUCKING_FILE]/100; }
        cache_size[t.ss.ff] -= video_size[t.ss.ss];
        output[t.ss.ff].insert(t.ss.ss);
        changed.insert(t.ss.ss);
        cnt++;
        if (cnt >= FHKGJDKJLFBG[FUCKING_FILE]/10){
            cout<<ccnt<<"/"<<FHKGJDKJLFBG[FUCKING_FILE]<<"updated!"<<endl;
            update_scores();
            cnt = 0;
        }
    }
